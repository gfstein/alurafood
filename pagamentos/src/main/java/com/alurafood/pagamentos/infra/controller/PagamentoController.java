...
public class PagamentoController {
    ...
    public Page<PagamentoDto> listar(@PageableDefault() Pageable pageable) {
        return service.getAllPagamentos(pageable)
                .map(PagamentoDto::fromModel);
    }

    @GetMapping("/{id}")
    public ResponseEntity<PagamentoDto> detalhar(@PathVariable @NotNull UUID id) {
        return ResponseEntity.ok(PagamentoDto.fromModel(service.getPagamentoById(id)));
    }

    @PostMapping
    public ResponseEntity<PagamentoDto> cadastrar(@RequestBody @Valid PagamentoDto dto, UriComponentsBuilder uriBuilder) {
        Pagamento pagamento = service.criarPagamento(Pagamento.of(dto));
        URI uri = uriBuilder.path("/pagamentos/{id}").buildAndExpand(pagamento.getId()).toUri();
        return ResponseEntity.created(uri).body(PagamentoDto.fromModel(pagamento));
    }

    @PutMapping("/{id}")
    public ResponseEntity<PagamentoDto> atualizar(@PathVariable @NotNull UUID id, @RequestBody @Valid PagamentoDto dto) {
        return ResponseEntity.ok(PagamentoDto.fromModel(service.atualizarPagamento(id, Pagamento.of(dto))));
    }
